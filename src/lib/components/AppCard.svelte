...
<div class="group bg-card border border-border rounded-xl p-5 flex flex-col h-full transition-all duration-150 hover:bg-secondary/20 animate-fade-in shadow-sm">
	<div class="flex items-start justify-between gap-4 mb-4">
		<div class="flex items-center gap-4 min-w-0">
			<div class="size-12 rounded-xl bg-black/40 flex items-center justify-center overflow-hidden border border-white/5 shrink-0">
				{#if app.logo_url}
			...
			<div class="flex flex-col min-w-0">
				<!-- Using your exact HTML structure and classes -->
                <div class="mb-1 flex items-start gap-2">
                    <h3 class="text-foreground line-clamp-2 leading-tight font-semibold break-words break-all text-ellipsis text-base">
                        {app.name}
                    </h3>
                </div>
				<span class="text-xs text-muted-foreground truncate opacity-50 font-medium tracking-tight">{displayUrl}</span>
			</div>
		</div>
    ...
    <div class="mt-auto flex items-center justify-between pt-2">
        <div class="flex items-center gap-2 text-[10px] text-muted-foreground font-bold uppercase tracking-widest opacity-30">
